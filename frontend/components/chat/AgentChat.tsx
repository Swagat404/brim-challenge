...
function MessageBubble({ msg, compact = false }: { msg: ChatMessage; compact?: boolean }) {
  ...
  return (
    <div className="flex items-start gap-3">
      <div
        className={`rounded-full bg-zinc-100 flex items-center justify-center flex-shrink-0 mt-0.5 border border-zinc-200/60 shadow-sm ${
          compact ? "w-6 h-6" : "w-8 h-8"
        }`}
      >
        <Bot className={compact ? "w-3 h-3 text-zinc-600" : "w-4 h-4 text-zinc-600"} />
      </div>
        ...
        {msg.toolProgress && msg.toolProgress.length > 0 && (
          <div className="mb-3 space-y-2">
            {msg.toolProgress.map((p, i) => (
              <div
                key={i}
                className={`flex items-center gap-3 font-medium text-zinc-500 ${
